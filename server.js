...
var defaultPageSlugs = { home: "", about: "about", products: "products", export: "export", contacts: "contacts", privacy: "privacy", terms: "terms" };
...
function getManagedPageSlug(pageId, pageSeo = defaultPageSeo) {
  if (pageId === "home") return "";
  return normalizeSlug(pageSeo[pageId]?.slug || "", defaultPageSlugs[pageId]);
}
function getManagedPagePath(pageId, pageSeo = defaultPageSeo) {
...
function asContentString(value, fallback = "") {
...
async function findProductRowByIdentifier(identifier) {
...
function mapProductsPage(row) {
  return {
    ...
    detailUi: { ...defaultProductsPage.detailUi, ...safeParseJson(row?.detail_ui, defaultProductsPage.detailUi) },
...
function getProductPath(product, pageSeo = defaultPageSeo) {
  return `${getManagedPagePath("products", pageSeo)}/${getManagedProductSlug(product)}`;
}
function resolveStaticPageByPath(pathname, pageSeo) {
...
async function resolveProductPath(pathname, pageSeo) {
  const normalizedPath = normalizePathname(pathname);
...
async function validatePageSeoInput(pageId, payload) {
  ...
  for (const otherPageId of Object.keys(defaultPageSeo)) {
...
async function validateProductPayload(product, existingId = "") {
  ...
  const duplicate = res.rows.find((row) => {
    ...
    const rowSlug = normalizeSlug(asString(safeParseJson(row?.seo, {}).slug), normalizeSlug(rowId, rowId));
...
function renderHtmlWithSeo(template, meta) {
...
async function buildSeoMeta(req) {
  ...
  const toCanonicalUrl = (pathname) => `${origin}${pathname === "/" ? "" : pathname}`;
  const fromPage = (pageId, canonicalPath, robots = "index,follow") => ({
    statusCode: 200,
...
app.post("/api/leads/:id", async (req, res) => {
  try {
    const status = asString(req.body?.status, "New");
...
app.get("/sitemap.xml", async (req, res) => {
  try {
    const origin = getOrigin(req);
    const pageSeo = await getPageSeo();
    const staticUrls = Object.keys(defaultPageSeo).map((pageId) => `${origin}${getManagedPagePath(pageId, pageSeo) === "/" ? "" : getManagedPagePath(pageId, pageSeo)}`);
