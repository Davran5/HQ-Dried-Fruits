...
const uzUiLabels = {
    ...
    qualityGuaranteeTitle: "Sifat kafolati", qualityGuaranteeDesc: "Bizning quvvatlarimiz 99.9% tozalikni ta'minlash uchun lazerli saralashdan foydalanadi.",
...
// Replace the translations object in server.ts
const translationsRegex = /const translations: Record<string, any> = \{([\s\S]*?)\};/m;
const newTranslationsContent = `const translations: Record<string, any> = {
      ru: {
        global_settings: {
          ...
          ui_labels: JSON.stringify(${JSON.stringify(ruUiLabels, null, 10)})
        },
        home_page: {
      ...
      uz: {
        global_settings: {
          ...
          ui_labels: JSON.stringify(${JSON.stringify(uzUiLabels, null, 10)})
        },
        home_page: {
          content: JSON.stringify({
...
content = content.replace(translationsRegex, newTranslationsContent);

fs.writeFileSync(filePath, content);
console.log('Successfully updated server.ts translations');
